...
export default function AdminDashboard() {
  ...
  return (
    <div className="space-y-4">
      {/* Header row — Create button top right */}
      <div className="flex items-center justify-end">
        <Button
          asChild
          className="bg-[#1DA1F2] hover:bg-[#1a91da] rounded-full px-5"
        >
      ...
      <div className="md:hidden space-y-3">
        {isLoading
          ...
          : filtered.map((listing, i) => (
              <div
                key={listing.id}
                className="bg-white border rounded-xl px-4 py-3 cursor-pointer active:bg-slate-50 transition-colors"
                onClick={() => navigate(`/listings/${listing.id}`)}
              >
                <div className="flex items-start justify-between gap-2 mb-1.5">
                  ...
                  <span
                    className={`text-xs px-2.5 py-0.5 rounded-full font-medium ${listcaseStatusColor[listing.listcaseStatus]}`}
                  >
